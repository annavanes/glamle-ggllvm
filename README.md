...
Simulation code for Model 4 with Poisson assumption , i.e. with the linear predictor $\eta_{ij}^{(k)} = \boldsymbol{\alpha}_{ij}^T \boldsymbol{z}^{(k)} + \boldsymbol{\beta}_{ij}^T \boldsymbol{x}_{ij}^{(k)}$.
